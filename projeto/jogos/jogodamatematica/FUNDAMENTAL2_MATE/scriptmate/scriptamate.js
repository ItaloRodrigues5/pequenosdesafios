...
function ProximaPergunta() {
    ...
    const num1 = Math.floor(Math.random() * 11); // Números de 0 a 10
    let num2;

    if (Operacao === '-') {
        num2 = Math.floor(Math.random() * (num1 + 1)); // Garante que num2 não seja maior que num1
    } else if (Operacao === '/') {
        num2 = Math.floor(Math.random() * (num1 + 1)) + 1; // Garante que num2 não seja zero
    } else {
        num2 = Math.floor(Math.random() * 11); // Números de 0 a 10 para soma e multiplicação
    }
...
function calcularResposta(num1, num2, operacao) {
    switch (operacao) {
        ...
        case '*':
            return num1 * num2;
        case '/':
            return Math.floor(num1 / num2); // Usamos Math.floor para garantir um resultado inteiro
        default:
            return null;
    }
}
...
function calcularScore(Resultado) {
    ...
    if (Resultado >= 51 && Resultado <= 10000) return 250;
}